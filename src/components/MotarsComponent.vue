<template>
    ...
    <div class="min-h-screen w-full content-center justify-center p-5 bg-gray-200">
        <div class="grid grid-cols-4 gap-3">
            <div v-for="(motor, index) in data" :key="index" class="w-96 bg-white p-3">
                <img class="h-52 w-full object-cover" :src="motor.url"/>
...
import { DataService } from '@/service/DataService'
    ...
    const fetchCountyData = async () => {
      try {
        data.value = await dataService.getMotorBike();
